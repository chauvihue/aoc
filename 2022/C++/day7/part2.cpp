...
int main()
{
    ...
    dfs1();
    needed = MINNEEDED - (SYSTEMSIZE - sz[1]);
    res = sz[1];
    dfs2();
    /*
...
void dfs2(int id)
{
    //cerr << name_lookup[id] << '|' << res << '\n';
    for (int i = 0, child; i < hierarchy[id].size(); i++)
    {
        child = hierarchy[id][i];
        if (hierarchy[child].empty())
            continue;
        if (sz[child] >= needed)
        {
            res = min(res, sz[child]);
            //cerr << name_lookup[child] << "!\n";
            dfs2(child);
        }
    }
}
